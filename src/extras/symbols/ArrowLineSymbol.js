...
function(
declare, lang, query, dom, construct, style, gfx, screenUtils, SimpleLineSymbol, SimpleMarkerSymbol, PictureMarkerSymbol, graphic, Point, ScreenPoint, bfx, dfx, xfx, on) {
  return declare([esri.symbols.SimpleLineSymbol],
  /**  @lends module:extras/symbol/ArrowLineSymbol */
  {
    ...
    _drawDirection: function(graphic, graphicsLayer, map) {
      ...
      var screenGeo = esri.geometry.screenUtils.toScreenGeometry(map.extent, map.width, map.height, geometry);
      var screenExtent = esri.geometry.screenUtils.toScreenGeometry(map.extent, map.width, map.height, map.extent);
        ...
        jLen = line.length - 1; j < jLen; j++) {
          ...
          xLen = directionPoints.length; x < xLen; x++) {
            ...
            if (this.directionSymbol.type === "simplemarkersymbol" || this.directionSymbol.type === "picturemarkersymbol") {
              sym = dojo._base.lang.clone(this.directionSymbol);
            }
            else if (typeof this.directionSymbol === "string") {

              sym = new esri.symbols.SimpleMarkerSymbol();
            ...
            var sp = new esri.geometry.ScreenPoint(directionPoints[x][0], directionPoints[x][1]);
            var mp = map.toMap(sp);
            g.geometry = mp;
            graphicsLayer.add(g);
