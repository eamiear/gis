...
* @requires dojo.fx 
*/
define(["esri/graphic", "esri/symbols/PictureMarkerSymbol", "esri/symbols/SimpleMarkerSymbol", "esri/SpatialReference", "esri/layers/GraphicsLayer", "dojo/fx"],
function(
graphic, PictureMarkerSymbol, SimpleMarkerSymbol, SpatialReference, GraphicsLayer, fx) {
  return declare(null,
  /**  @lends module:extras/control/LayerLocate */
  {
    ...
    initLayerLocate: function(map) {
      this.map = map;
      if (!this.locateLayer) {
        this.locateLayer = new esri.layers.GraphicsLayer({
    ...
    locate: function(geometry, isCenter, isEffect, zoom, style, fn, endStatic) {
      var center = null;
      if (geometry) {
        geometry = esri.geometry.webMercatorUtils.geographicToWebMercator(geometry);
        center = geometry;
